...
def test_simulate_measurements():
    data = torch.load('/dccstor/gridfm/opf_data/case14_ieee/processed/data_index_0.pt', weights_only=False)
    node_stats = torch.load('/dccstor/gridfm/opf_data/case14_ieee/processed/data_stats_HeteroDataMVANormalizer.pt', weights_only=False)
    ...
    assert torch.isclose(out['bus'].x[:, VA_H+1:], data['bus'].x[:, VA_H+1:]).all()
    assert torch.isclose(out['gen'].x[:, PG_H+1:], data['gen'].x[:, PG_H+1:]).all()
    ...
    mask_branch_small = out.mask_dict['branch'][:, :out.mask_dict['std_branch'].size(1)]
